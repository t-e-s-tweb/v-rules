#!/bin/bash
# Script to apply the custom outbound patch to v2rayNG
# Run this from the root of the v2rayNG repository

echo "Applying custom outbound support patch..."

# Check if we're in the right directory
if [ ! -f "V2rayNG/app/src/main/java/com/v2ray/ang/dto/RulesetItem.kt" ]; then
    echo "Error: Not in v2rayNG repository root or files not found"
    exit 1
fi

echo "Step 1: Patching RulesetItem.kt..."
cat > /tmp/ruleset_item.patch << 'EOF'
...
EOF
git apply /tmp/ruleset_item.patch || echo "RulesetItem.kt patch failed, may need manual application"

echo "Step 2: Patching arrays.xml..."
cat > /tmp/arrays.patch << 'EOF'
...
EOF
git apply /tmp/arrays.patch || echo "arrays.xml patch failed, may need manual application"

echo "Step 3: Patching activity_routing_edit.xml..."
cat > /tmp/layout.patch << 'EOF'
...
+            </LinearLayout>
            <LinearLayout
                ...
                android:layout_height="wrap_content"
EOF
git apply /tmp/layout.patch || echo "activity_routing_edit.xml patch failed, may need manual application"

echo "Step 4: Patching RoutingEditActivity.kt..."
cat > /tmp/routing_edit.patch << 'EOF'
...
EOF
git apply /tmp/routing_edit.patch || echo "RoutingEditActivity.kt patch failed, may need manual application"

echo "Step 5: Patching V2rayConfigManager.kt..."
cat > /tmp/config_manager.patch << 'EOF'
...
EOF
git apply /tmp/config_manager.patch || echo "V2rayConfigManager.kt patch failed, may need manual application"

echo ""
echo "Patch application complete!"
echo "If any patches failed, you may need to apply them manually."
